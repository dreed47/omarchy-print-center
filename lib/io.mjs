...
// CUPS's own page_log carries a "total N" line per job with the real page
// count - independent of printer-state-reasons, which often clear back to
// "none" within seconds of a backend failure, well before the next poll.
// A job that shows "completed" everywhere else but printed 0 pages is
// exactly the case that otherwise looks indistinguishable from success (see
// printLogic.zeroPageAlerts). World-readable (mode 644, root:nobody) - no
// elevation needed. Bounded to a generous tail so a long-lived machine's
// log can't grow this read without limit.
const PAGE_LOG_PATH = "/var/log/cups/page_log"
const MAX_PAGE_LOG_READ_BYTES = 256 * 1024
export function readPageLogTail() {
    try {
        const buf = readFileSync(PAGE_LOG_PATH)
        return buf.length > MAX_PAGE_LOG_READ_BYTES
            ? buf.subarray(buf.length - MAX_PAGE_LOG_READ_BYTES).toString("utf8")
            : buf.toString("utf8")
    } catch {
        return ""   // missing, unreadable, or page logging disabled - just means no signal
    }
}
