...
import { readFileSync, mkdirSync, readdirSync, rmSync, renameSync, copyFileSync, unlinkSync, existsSync, lstatSync, realpathSync } from "node:fs"
...
// How this plugin copy is installed, which decides whether self-update is safe.
//   "symlink" - a dev checkout linked into the plugins dir; never touch it
//   "git"     - a real dir with .git; `git checkout <tag>` works
//   "copy"    - a plain dir, no .git; can only point the user at the release
//
// `pluginDir` is the resolved directory the code runs from (Qt already
// follows symlinks). `linkDir` is ~/.config/omarchy/plugins - if any entry
// there is a symlink resolving to pluginDir, this is a linked dev install.
export function detectInstallKind(pluginDir, linkDir) {
    let real
    try { real = realpathSync(pluginDir) } catch { return "unknown" }

    if (linkDir) {
        try {
            for (const name of readdirSync(linkDir)) {
                const p = linkDir + "/" + name
                try {
                    if (lstatSync(p).isSymbolicLink() && realpathSync(p) === real) return "symlink"
                } catch {}
            }
        } catch {}
    }
    try {
        if (lstatSync(pluginDir).isSymbolicLink()) return "symlink"
    } catch {}
    return existsSync(real + "/.git") ? "git" : "copy"
}

const git = (dir, args, opts) => run("git", ["-C", dir, ...args], { timeout: 30000, ...opts })

export async function gitWorkingTreeClean(dir) {
    const { code, stdout } = await git(dir, ["status", "--porcelain"])
    return code === 0 && stdout.trim() === ""
}
export const gitFetchTags = (dir) => git(dir, ["fetch", "--tags", "--quiet", "origin"], { timeout: 45000 })
export async function gitLatestTag(dir) {
    const { code, stdout } = await git(dir, ["tag", "-l", "v*", "--sort=-v:refname"])
    if (code !== 0) return ""
    return stdout.split("\n").map((s) => s.trim()).filter(Boolean)[0] || ""
}
export const gitCheckoutTag = (dir, tag) =>
    git(dir, ["-c", "advice.detachedHead=false", "checkout", "--quiet", tag])
